...
program
  ...
  .action(async (projectName) => {
    try {
      ...
      if(!framework){
        console.error(COLORS.FgRed, 'Error al clonar seleccionar framework.');
        shell.exit(1);
      }
      ...
      console.log(COLORS.FgGreen, 'Proyecto creado exitosamente.');
    } catch (error) {
      console.log(COLORS.FgRed, 'El Proyecto no se creo: ',error?.message);
    }
  });
...
program
  ...
  .action(() => {
    try {
      ...
      console.log(COLORS.FgRed, 'Error al actualizar: ',error?.message);
    }
  });
